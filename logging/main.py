...
def with_execution_environment(original):
    ...
    def decorated(logger, *args, **kwargs):
        ...
        # Define the execution environment filter to add the execution environment
        # parameters to the LogRecord
        ...
        # Forward the Logger with the added execution environment filter to the original
        # function
        result = original(logger, *args, **kwargs)
        # Remove the execution environment filter from the Logger to prevent the
        # execution environment parameters to appear in logs outside the scope of the
        # function on which the @with_execution_environment decorator has been applied
...
def with_execution_context(original):
    ...
    def decorated(logger, request, *args, **kwargs):
        ...
        # Define the execution context filter to add the execution context parameters to
        # the LogRecord
        ...
        # Forward the Logger with the added execution context filter to the original
        # function
        result = original(logger, request, *args, **kwargs)
        # Remove the execution context filter from the Logger to prevent the execution
        # context parameters # to appear in logs outside the scope of the function on
        # which the @with_execution_context decorator has been applied
