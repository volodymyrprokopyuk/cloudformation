...
function deploy_infringement_ingest_stack {
    ...
    aws cloudformation $stack_action --stack-name $stack_name \
        ...
        ParameterKey=IamSmartProtectionAccountId,ParameterValue=$SMART_PROTECTION_ACCOUNT_ID \
        ParameterKey=IamSmartProtectionExternalId,ParameterValue=$SMART_PROTECTION_EXTERNAL_ID \
...
function deploy_infringement_expose_stack {
    ...
    aws cloudformation $stack_action --stack-name $stack_name \
        ...
        ParameterKey=RdsEndpointPortExportName,ParameterValue=$APPLICATION-store-$ENVIRONMENT:RdsEndpointPort \
        ParameterKey=DbName,ParameterValue=$DB_NAME \
        ParameterKey=DbUser,ParameterValue=$DB_EXPOSE_USER \
        ParameterKey=DbPassword,ParameterValue=$DB_EXPOSE_PASSWORD \
        ParameterKey=DbConnectTimeout,ParameterValue=$DB_CONNECT_TIMEOUT \
        ParameterKey=S3LambdaPackageBucketName,ParameterValue=$S3_EXPOSE_LAMBDA_PACKAGE_BUCKET_NAME \
        ParameterKey=LambdaSecurityGroupIdExportName,ParameterValue=$APPLICATION-store-$ENVIRONMENT:LambdaSecurityGroupId \
        ParameterKey=PrivateSubnetAIdExportName,ParameterValue=$APPLICATION-store-$ENVIRONMENT:PrivateSubnetAId \
        ParameterKey=PrivateSubnetBIdExportName,ParameterValue=$APPLICATION-store-$ENVIRONMENT:PrivateSubnetBId \
        ParameterKey=LambdaLogLevel,ParameterValue=$LAMBDA_LOG_LEVEL \
        ParameterKey=LambdaGetPartnerFromDbVersion,ParameterValue=$LAMBDA_GET_PARTNER_FROM_DB_VERSION \
        ParameterKey=LambdaGetProductFromDbVersion,ParameterValue=$LAMBDA_GET_PRODUCT_FROM_DB_VERSION \
        ParameterKey=LambdaGetPirateSourceFromDbVersion,ParameterValue=$LAMBDA_GET_PIRATE_SOURCE_FROM_DB_VERSION \
        ParameterKey=LambdaGetInfringementFromDbVersion,ParameterValue=$LAMBDA_GET_INFRINGEMENT_FROM_DB_VERSION \
        ParameterKey=LambdaPostInfringementInDbVersion,ParameterValue=$LAMBDA_POST_INFRINGEMENT_IN_DB_VERSION \
        ParameterKey=ApiGatewayExposeInfringementDataRestApiVersion,ParameterValue=$API_VERSION \
        --capabilities CAPABILITY_NAMED_IAM \
        --tags \
        Key=Owner,Value=$OWNER
    wait_for_cf_desired_stack_status $stack_name is_cf_stack_create_update_complete
