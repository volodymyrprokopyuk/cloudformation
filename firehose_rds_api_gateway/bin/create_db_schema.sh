...
readonly SQL_FORCE_OTHER_SESSIONS_DISCONNECT="
...
readonly SQL_DROP_ROLE="DROP ROLE IF EXISTS ingest, ingest_role, expose, expose_role;"
...
-- Create expose_role with privileges
CREATE ROLE expose_role;
GRANT USAGE ON SCHEMA ingest TO expose_role;
GRANT SELECT ON ALL TABLES IN SCHEMA ingest TO expose_role;
GRANT SELECT, INSERT, UPDATE ON TABLE ingest.infringement TO expose_role;
GRANT ALL ON ALL SEQUENCES IN SCHEMA ingest TO expose_role;

-- Create expose user and grant him expose_role privileges
CREATE ROLE expose WITH PASSWORD '$DB_EXPOSE_PASSWORD' LOGIN;
GRANT expose_role TO expose;
EOF
