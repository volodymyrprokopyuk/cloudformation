...
class JsonFormatter:  # pylint: disable=too-few-public-methods
    ...
    OPTIONAL_JSON_ATTRIBUTES = ["stack", "version", "document", "request_id"]
...
def log_document_context(original):
    """Add ingest document context-specific parameters to a LogRecord"""
...
def log_request_context(original):
    """Add expose data context-specific parameters to a LogRecord"""

    @wraps(original)
    def decorated(logger, f1, event, context, *args, **kwargs):
        request_id = get_or_default(
            event, "requestContext.requestId", str(uuid.uuid4())
        )
